    ...
    def generate_variables_tf(self, instance: ResourceInstanceIR) -> str:
        ...
        if instance.config.force_destroy is not None:
            parts.append(self._r.render_variable(
                "force_destroy", "bool", "Allow deletion of non-empty bucket by deleting all objects",
                default=instance.config.force_destroy,
            ))
        if instance.config.object_lock_enabled is not None:
            parts.append(self._r.render_variable(
                "object_lock_enabled", "bool", "Enable S3 Object Lock on the bucket",
                default=instance.config.object_lock_enabled,
            ))
        if instance.config.acceleration_status is not None:
            parts.append(self._r.render_variable(
                "acceleration_status", "string", "Transfer acceleration status for the bucket",
                default=instance.config.acceleration_status,
            ))
        if instance.config.tags is not None:
            parts.append(self._r.render_variable(
                "tags", "map(string)", "Tags to apply to the S3 bucket",
            ))
        return "\n".join(parts)
