...
class EKSGenerator:
    ...
    def generate_variables_tf(self, instance: ResourceInstanceIR) -> str:
        ...
        parts = [
            self._r.render_variable("cluster_name", "string", "Name of the EKS cluster"),
            self._r.render_variable("cluster_role_arn", "string", "ARN of the IAM role for the EKS cluster"),
            self._r.render_variable("subnet_ids", "list(string)", "List of subnet IDs for the EKS cluster VPC config"),
        ]
        if instance.config.eks_version is not None:
            parts.append(self._r.render_variable(
                "eks_version", "string", "Kubernetes version for the EKS cluster",
                default=instance.config.eks_version,
            ))
        if instance.config.eks_endpoint_public_access is not None:
            parts.append(self._r.render_variable(
                "eks_endpoint_public_access", "bool", "Whether the EKS cluster endpoint is publicly accessible",
