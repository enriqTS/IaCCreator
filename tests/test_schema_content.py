...
class TestVariableNamesPerService:
    ...
    def test_lambda_variables(self):
        expected = {
            ...
            "description", "environment_variables", "tags", "layers",
            "architectures", "ephemeral_storage_size",
            "reserved_concurrent_executions", "publish",
        }
        assert set(_names(ServiceType.LAMBDA)) == expected

    def test_s3_variables(self):
        expected = {
            "bucket_name", "versioning", "tags", "force_destroy",
            "object_lock_enabled", "acceleration_status",
        }
        assert set(_names(ServiceType.S3)) == expected

    def test_dynamodb_variables(self):
        expected = {
            "table_name", "billing_mode", "hash_key", "hash_key_type",
            "range_key", "range_key_type", "read_capacity", "write_capacity",
            "tags", "point_in_time_recovery_enabled",
            "deletion_protection_enabled", "table_class",
        }
    ...
    def test_cloudwatch_variables(self):
        expected = {
            "log_group_name", "retention_in_days", "tags",
...
class TestSpecificDefaults:
    ...
    def test_lambda_handler_default(self):
        assert _entry(ServiceType.LAMBDA, "handler").default == "lambda_function.lambda_handler"
...
class TestGroupAssignments:
    ...
    def test_lambda_general_group(self):
        assert _group_members(ServiceType.LAMBDA, "General") == {
            "function_name", "handler", "runtime", "description",
        }

    def test_lambda_performance_group(self):
        assert _group_members(ServiceType.LAMBDA, "Performance") == {
            "memory_size", "timeout", "ephemeral_storage_size",
            "reserved_concurrent_executions", "architectures",
        }
