...
class TestIRBuilderBasic:
    ...
    def test_build_groups_resources_by_service_type(self):
        desc = _make_input(
            resources=[
                ResourceInstance(name="func-a", service_type=ServiceType.LAMBDA, config=ResourceConfig(handler="a.handler", runtime="python3.12")),
                ResourceInstance(name="func-b", service_type=ServiceType.LAMBDA, config=ResourceConfig(handler="b.handler", runtime="python3.12")),
    ...
    def test_build_creates_environments_with_module_refs(self):
        desc = _make_input(
            resources=[
                ResourceInstance(name="func-a", service_type=ServiceType.LAMBDA, config=ResourceConfig(handler="a.handler", runtime="python3.12")),
                ResourceInstance(name="my-table", service_type=ServiceType.DYNAMODB, config=ResourceConfig(hash_key="id")),
            ],
            environments=[
    ...
    def test_build_preserves_environment_variables(self):
        desc = _make_input(
            environments=[EnvironmentConfig(name="dev", variables={"region": "us-east-1", "stage": "dev"})]
        )
        ir = IRBuilder().build(desc)
...
class TestConnectionValidation:
    ...
    def test_accepts_valid_api_gateway_to_lambda(self):
        desc = _make_input(
            resources=[
                ResourceInstance(name="my-api", service_type=ServiceType.API_GATEWAY, config=ResourceConfig(protocol_type="HTTP")),
                ResourceInstance(name="my-func", service_type=ServiceType.LAMBDA, config=ResourceConfig(handler="h", runtime="python3.12")),
            ],
            connections=[Connection(source="my-api", target="my-func", connection_type="triggers")],
        )
        ir = IRBuilder().build(desc)
...
class TestIAMStatementDerivation:
    ...
    def test_lambda_to_dynamodb_iam_statements(self):
        desc = _make_input(
            ...
            connections=[Connection(source="my-func", target="my-table", connection_type="reads_from")],
        )
        ir = IRBuilder().build(desc)
        lambda_module = next(m for m in ir.modules if m.service_type == ServiceType.LAMBDA)
    ...
    def test_lambda_to_s3_iam_statements(self):
        desc = _make_input(
            ...
            connections=[Connection(source="my-func", target="my-bucket", connection_type="writes_to")],
        )
        ir = IRBuilder().build(desc)
        lambda_module = next(m for m in ir.modules if m.service_type == ServiceType.LAMBDA)
        func_ir = lambda_module.instances[0]
    ...
    def test_lambda_to_cloudwatch_iam_statements(self):
        desc = _make_input(
            ...
            connections=[Connection(source="my-func", target="my-logs", connection_type="logs_to")],
        )
        ir = IRBuilder().build(desc)
        lambda_module = next(m for m in ir.modules if m.service_type == ServiceType.LAMBDA)
        func_ir = lambda_module.instances[0]
    ...
    def test_multiple_connections_produce_empty_iam_statements(self):
        desc = _make_input(
            ...
            connections=[
                Connection(source="my-func", target="my-table", connection_type="reads_from"),
                Connection(source="my-func", target="my-bucket", connection_type="writes_to"),
            ],
        )
        ir = IRBuilder().build(desc)
        lambda_module = next(m for m in ir.modules if m.service_type == ServiceType.LAMBDA)
    ...
    def test_api_gateway_to_lambda_no_iam_on_gateway(self):
        ...
        ir = IRBuilder().build(desc)
        api_module = next(m for m in ir.modules if m.service_type == ServiceType.API_GATEWAY)
