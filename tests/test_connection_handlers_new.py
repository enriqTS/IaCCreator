"""Unit tests for all connection handlers in the refactored architecture.
...
# ===========================================================================
# 1. API Gateway → Lambda (route_handler role)
# ===========================================================================


class TestHandleApigwLambda:
    """Test API Gateway → Lambda handler generates integration, route(s), and permission."""

    def setup_method(self):
        self.handler = ApiGatewayLambdaHandler()

    def _handle_apigw_lambda(self, connection_config=None):
        """Helper: create APIGW→Lambda connection and handle it directly."""
    ...
    def test_empty_routes_generates_two_files(self):
        """APIGW→Lambda with empty routes produces 2 files: integration + permission."""
        files = self._handle_apigw_lambda({"routes": []})
        assert len(files) == 2

    def test_no_routes_key_generates_two_files(self):
        """APIGW→Lambda with no routes key produces 2 files: integration + permission."""
        files = self._handle_apigw_lambda({})
        assert len(files) == 2
...
# ===========================================================================
# 1b. API Gateway → Lambda (authorizer role)
# ===========================================================================


class TestHandleApigwLambdaAuthorizer:
    """Test API Gateway → Lambda authorizer handler generates authorizer + permission."""
    ...
    def test_generates_two_files(self):
        ...
        files = self._handle_apigw_lambda_authorizer()
        assert len(files) == 2
    ...
    def test_route_handler_still_generates_three_files(self):
        """Explicit route_handler role with a route still produces 3 files."""
        apigw = _apigw_ir("my-api")
        func = _lambda_ir("my-func")
        conn = ConnectionIR(
            source_name="my-api",
...
class TestHandleLambdaSqs:
    ...
    def test_returns_no_files(self):
        ...
        files, _ = self._handle_lambda_sqs()
        assert files == []

    def test_attaches_iam_statement(self):
        """Lambda→SQS attaches exactly one IAM statement to the Lambda."""
        _, func = self._handle_lambda_sqs()
        assert len(func.iam_statements) == 1
...
class TestHandleSqsLambda:
    ...
    def setup_method(self):
        self.handler = SQSLambdaHandler()

    def _handle_sqs_lambda(self, connection_config=None):
        queue = _sqs_ir("my-queue")
        func = _lambda_ir("my-func")
        conn = ConnectionIR(
    ...
    def test_batch_size_uses_config_value(self):
        ...
        files, _ = self._handle_sqs_lambda(connection_config={"batch_size": 25})
        mapping_file = next(f for f in files if "event_source_" in f.path)
    ...
    def test_attaches_sqs_receive_iam_statements(self):
        ...
        _, func = self._handle_sqs_lambda()
        assert len(func.iam_statements) == 1
        stmt = func.iam_statements[0]
...
class TestHandleSnsSqs:
    ...
    def setup_method(self):
        self.handler = SNSSQSHandler()

    def _handle_sns_sqs(self):
        topic = _sns_ir("my-topic")
        queue = _sqs_ir("my-queue")
        conn = ConnectionIR(
...
class TestHandleSnsLambda:
    ...
    def setup_method(self):
        self.handler = SNSLambdaHandler()

    def _handle_sns_lambda(self):
        topic = _sns_ir("my-topic")
        func = _lambda_ir("my-func")
        conn = ConnectionIR(
            source_name="my-topic",
...
class TestHandleLambdaDynamoDBAccessPattern:
    ...
    def test_default_access_pattern_uses_all_actions(self):
        ...
        func = self._handle_lambda_dynamodb()
        assert len(func.iam_statements) == 1
    ...
    def test_full_access_pattern_uses_all_actions(self):
        ...
        func = self._handle_lambda_dynamodb({"access_pattern": "full"})
        assert len(func.iam_statements) == 1
        actions = func.iam_statements[0].actions
        assert "dynamodb:GetItem" in actions
        assert "dynamodb:Query" in actions
    ...
    def test_read_access_pattern_uses_only_read_actions(self):
        ...
        func = self._handle_lambda_dynamodb({"access_pattern": "read"})
        assert len(func.iam_statements) == 1
        actions = func.iam_statements[0].actions
    ...
    def test_write_access_pattern_uses_only_write_actions(self):
        ...
        func = self._handle_lambda_dynamodb({"access_pattern": "write"})
        assert len(func.iam_statements) == 1
        actions = func.iam_statements[0].actions
        assert actions == [
            "dynamodb:PutItem",
...
class TestHandleLambdaS3AccessPattern:
    ...
    def test_full_access_pattern_uses_all_actions(self):
        ...
        func = self._handle_lambda_s3({"access_pattern": "full"})
        assert len(func.iam_statements) == 1
        actions = func.iam_statements[0].actions
        assert "s3:GetObject" in actions
        assert "s3:PutObject" in actions
    ...
    def test_read_access_pattern_uses_only_read_actions(self):
        ...
        func = self._handle_lambda_s3({"access_pattern": "read"})
        assert len(func.iam_statements) == 1
        actions = func.iam_statements[0].actions
    ...
    def test_write_access_pattern_uses_only_write_actions(self):
        ...
        func = self._handle_lambda_s3({"access_pattern": "write"})
        assert len(func.iam_statements) == 1
        actions = func.iam_statements[0].actions
