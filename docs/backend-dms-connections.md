...
Use `source_secret_endpoint` or `target_secret_endpoint` for RDS MySQL, MariaDB, PostgreSQL, SQL Server Enterprise/Standard, Oracle Enterprise/Standard Edition 2 (including CDB variants), or Aurora MySQL/PostgreSQL. These four additional specifications use the same endpoint identifier, database name, and imported certificate fields; Oracle supplies a DMS-imported auto-login wallet through `certificate_arn`. Instead of `database_user`, supply `secrets_manager_arn` (a full secret ARN) and `secrets_manager_access_role_arn` (an existing IAM role ARN). Secret endpoints do not enable database IAM authentication or impose the IAM-specific DMS 3.6.1 minimum.
...
| `cdc_start_position` | Required native MySQL binlog position, PostgreSQL WAL LSN, SQL Server LSN, or Oracle SCN for CDC-only tasks |
...
RDS engines `oracle-ee`, `oracle-se2`, `oracle-ee-cdb`, and `oracle-se2-cdb` map to DMS `engine_name = "oracle"` through Secrets Manager source/target endpoints. They support stopped full-load sources and migration targets, including targets receiving CDC from supported MySQL, PostgreSQL, and SQL Server sources. Oracle IAM authentication, RDS Custom, and other Oracle edition identifiers remain unsupported here.

Oracle uses `verify-ca` TLS with an **auto-login Oracle wallet imported into DMS**, referenced by `certificate_arn`. A PEM certificate import alone is insufficient. Prepare and import the wallet separately, in the replication instance's account and Region, following [DMS Oracle SSL setup](https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Source.Oracle.html#CHAP_Source.Oracle.SSL). The generator validates the ARN shape/account/Region relationship, but does not inspect or create wallet contents, read credentials, or configure the database TLS listener.

Populate the external secret with the database host, TLS listener port, username, and password. Select the Oracle database service in `database_name`; for a CDB engine, select the intended tenant PDB, not the CDB root. RDS connects clients to PDBs as described in [RDS CDB limitations](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Oracle.Concepts.CDBs.html). Simple service identifiers must fit the existing connection field's identifier format; arbitrary connect descriptors are not exposed.

Table selections and destination transformations preserve case. Supply Oracle schema/table names as stored in the database (commonly uppercase), and prepare matching destination schemas and empty tables for full load. SQL grants, compatible Oracle/DMS versions, database provisioning/licensing, wallet rotation, and network reachability remain deployment prerequisites. Review [Oracle source requirements](https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Source.Oracle.html) and [Oracle target requirements](https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Target.Oracle.html) before starting a task. Legacy Binary Reader path mapping, automatic database logging configuration, ASM/TDE credentials, schema conversion, and live migration validation are outside this unit.


### Oracle LogMiner CDC

Non-CDB `oracle-ee` and `oracle-se2` secret sources support `full-load-and-cdc` and `cdc`. Endpoints use DMS’s default LogMiner reader without extra connection attributes. CDB/PDB CDC requires selecting Binary Reader as described below; full-load CDB sources and CDB migration targets remain supported. Terraform task preconditions restrict source engine overrides to the non-CDB editions. See [Oracle CDC readers](https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Source.Oracle.html).

CDC-only tasks require a positive decimal `cdc_start_position` SCN, without signs, leading zeros, or separators. Validation checks syntax, not database-specific SCN bounds or log availability. Select an SCN that covers the earliest open transaction and agrees with the target snapshot; the current SCN alone can miss open transactions. Full-load-plus-CDC omits an explicit position. Automatic SCN discovery and open-transaction-window configuration are not exposed. See [Oracle native start points](https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Task.CDC.html).

Prepare archive-log retention, supplemental logging, and the RDS Oracle DMS user grants externally. Verify database/DMS version compatibility and LogMiner limitations for the selected workload before starting the stopped task. The generator does not execute SQL setup or validate a live migration. Wallet, TLS listener, secret-role permissions, explicit table selection, and destination preparation requirements still apply.


### Oracle Binary Reader CDC

Set `oracle_cdc_reader = "binary-reader"` on the Secrets Manager source endpoint. This is required for `oracle-ee-cdb` and `oracle-se2-cdb` CDC and optional for non-CDB editions. `logminer` explicitly selects the existing non-CDB reader; omit the field to retain DMS defaults. Reader options are rejected on other database engines and on target endpoints.

The generator emits the fixed attributes `useLogminerReader=N;useBfile=Y;`, following the documented RDS Oracle 12.2/18/19 profile. Legacy 11.2/12.1 path replacement, standby-specific settings, and ASM credentials are not exposed. Verify the actual database version externally; the diagram does not model Oracle engine versions. Create `ONLINELOG_DIR` and `ARCHIVELOG_DIR`, grant read access and `DBA_DIRECTORIES` access, and prepare logging and backups before testing the endpoint. Binary Reader does not support RDS Oracle TDE-encrypted tablespaces. See [RDS Binary Reader setup and limitations](https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Source.Oracle.html).

Select the tenant PDB service for CDB sources. Both CDC modes keep native SCNs, explicit table selection, TLS wallet references, and stopped execution. Changing readers requires restarting the task. Oracle CDC rejects source schema/table names longer than 30 ASCII bytes; validate unmodeled column, constraint, and key names externally against the same limit. Full-load-only selections retain the general 63-character limit. Terraform tests verify configuration and dependency graphs, not a live migration.
