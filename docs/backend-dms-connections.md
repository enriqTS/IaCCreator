...
Endpoints retain `verify-ca` TLS and certificate account/Region checks. Identifiers are unique across both authentication methods. Tasks can combine IAM and secret endpoints; MySQL-family sources support the existing CDC modes, and PostgreSQL secret sources support the logical-replication modes below. Since external secret contents are not inspected, verify that they match diagram database identities before running a task.
