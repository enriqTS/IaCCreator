...
- [-] Phase 5 databases and application access: Lambda/ECS access to DynamoDB, Kinesis, named Keyspaces/Timestream tables, Neptune graph queries, OpenSearch document APIs, RDS/Aurora and MemoryDB IAM login, DocumentDB IAM client bindings, MSK topic consumers/producers, ActiveMQ and standalone ElastiCache client endpoint bindings, serverless Valkey/Redis IAM access and placement, and DMS network placement, RDS/Aurora IAM and Secrets Manager endpoints, and stopped full-load and MySQL/PostgreSQL/SQL Server/non-CDB Oracle-source CDC tasks with explicit table mappings and destination schema/prefix transformations exist. RDS SQL Server Enterprise/Standard secret endpoints support full-load and CDC sources with native LSNs and DMS version guards, plus migration targets. RDS Oracle EE/SE2 secret endpoints (including CDB variants) support full-load sources and migration targets; Non-CDB Oracle sources also support LogMiner CDC with native SCNs; CDB/PDB Binary Reader CDC and additional endpoint engines remain.
...
The generator registry contains 119 Terraform-capable service types, while the connection registry contains 164 connection specifications involving 64 services.
...
- [-] connection-owned service-role creation: EC2 secret access owns an instance role and profile; other service roles remain;
- [-] DNS aliases and validation records: load-balancer and CloudFront aliases exist; certificate validation records remain.
...
- [-] DMS → source database: RDS/Aurora IAM and Secrets Manager endpoints implemented, including RDS SQL Server Enterprise/Standard and Oracle EE/SE2 (including CDB) secret endpoints; non-CDB Oracle LogMiner CDC is implemented; CDB/PDB Binary Reader CDC and additional engines remain.
