...
- [-] Phase 5 databases and application access: Lambda/ECS access to DynamoDB, Kinesis, named Keyspaces/Timestream tables, Neptune graph queries, RDS/Aurora IAM database login, and DMS network placement exist; the other database integrations and DMS endpoints remain.
...
The generator registry contains 118 Terraform-capable service types, while the connection registry contains 134 connection specifications involving 59 services.
