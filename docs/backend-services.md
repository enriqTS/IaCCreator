...
Athena → S3 (`stores_results`) supplies a managed result URI and optional key prefix to the workgroup. Workgroup configuration enforces this location by default; query callers retain responsibility for S3/KMS authorization. Lake Formation → S3 (`registers`) supplies a bucket or prefix ARN to the registration node, using its service-linked or explicitly configured external role. Each node accepts one managed location; duplicate connectors are idempotent, conflicting locations are rejected, and unconnected raw locations remain usable. These connections do not create Glue catalog objects or grant query principals access.
